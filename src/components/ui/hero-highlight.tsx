...
export const HeroHighlight = ({
  ...
  return (
    <div className={cn('group relative h-screen w-full', containerClassName)} onMouseMove={handleMouseMove}>
      ...
      <motion.div
        className='pointer-events-none absolute inset-0 opacity-0 transition duration-300 bg-dot-thick-[#f37a0c] group-hover:opacity-100'
        style={{
...
export const Highlight = ({ children, className }: { children: React.ReactNode; className?: string }) => {
  return (
    <motion.span
      ...
      className={cn(`bg-gradient-to-r relative inline-block rounded-lg`, className)}
    >
      {children}
    </motion.span>
  )
